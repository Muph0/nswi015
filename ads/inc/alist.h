...
#define alist_ref(list, T, index) (*((T*)alist_ref_safe(list, index, sizeof(T))))
...
#define alist_append(list, T, item) do { \
        T _copy = item; \
        alist_append_safe (list, sizeof(T), &_copy); \
    } while (0);
...
#define alist_insert(list, T, index) (*(T*)alist_insert_safe(list, index, sizeof(T)))

void alist_remove_safe(alist_t list, alist_size_t index, alist_size_t itemSize);
#define alist_remove(list, T, index) alist_remove_safe(list, index, sizeof(T));
...
#define alist_pop_back(list, T, item) (alist_remove(list, T, alist_count(list)-1))
#define alist_pop_front(list, T, item) (alist_remove(list, T, 0))

void* alist_begin_safe(alist_t list, alist_size_t itemSize);
void* alist_end_safe(alist_t list, alist_size_t itemSize);

#define alist_begin(list, T) (*((T*)alist_begin_safe(list, sizeof(T))))
#define alist_end(list, T) (*((T*)alist_end_safe(list, sizeof(T))))
